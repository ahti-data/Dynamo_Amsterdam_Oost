...
- `templates/` — built-in think-cell `.pptx` slide templates for the "Download slide" export.
  ...
  choropleth and the venn have no think-cell equivalent and keep a plain xlsx download
  (the venn none at all). Adding a chart means repeating that ui/server pair — see
...
no `readxl`) the same suite runs **FAIL 0 | PASS 483 | SKIP 9**.
