...
export interface VerantwoordingSection {
  id: string
  title: string
  /** 'general' = alleen op het volledige Verantwoording-tabblad; anders de tabbladen die deze sectie in hun footnote tonen */
  relatedViews: ViewId[] | 'general'
  render: (state: AppState, ds: Dataset) => ReactNode
}

/**
 * Enige bron voor de verantwoordingstekst — gerenderd op het volledige
 * Verantwoording-tabblad (alle secties) én, gefilterd op relatedViews, als
 * compacte "Bronnen & verantwoording"-footnote onderaan de betreffende tabbladen.
 */
export const VERANTWOORDING_SECTIONS: VerantwoordingSection[] = [
  {
    id: 'afbakening',
    title: 'Afbakening',
    relatedViews: 'general',
    render: (state) => (
      <ul>
        <li>
  ...
  {
    id: 'samenhang-analyse',
    title: 'Samenhang-analyse (gezondheid & welzijn)',
  ...
  {
    id: 'vooruitblik-prognose',
    title: 'Vooruitblik (prognose)',
    relatedViews: ['vooruitblik'],
    render: () => (
      <ul>
        <li>
...
export function sectionsForView(viewId: ViewId): VerantwoordingSection[] {
  return VERANTWOORDING_SECTIONS.filter(
    (s) => Array.isArray(s.relatedViews) && s.relatedViews.includes(viewId),
  )
}
